...
func _ready():
	...
	gameList.connect("item_selected", Callable(self, "_game_selected"))
	gameList.connect("item_activated", Callable(self, "_game_activated"))
	gameInfo.hide()
	
	gocoNet.connect("info_received", Callable(self, "_info_received"))
	gocoNet.connect("games_received", Callable(self, "_games_received"))
	gocoNet.connect("bad_response", Callable(self, "_on_bad_response"))
	gocoNet.connect("download_success", Callable(self, "_on_download_success"))
	gocoNet.get_info()
...
func _game_selected(game_id):
	...
	gameInfo.find_child("Title").text = game.title
	gameInfo.find_child("VersionValue").text = str(game.version)
	gameInfo.find_child("AuthorValue").text = str(game.author)
