...
func _init():
	escript = EScript.new()
	console = load("res://Console/Console.tscn").instantiate()
	coder = load("res://Coder/Coder.tscn").instantiate()
	spriter = load("res://Spriter/Spriter.tscn").instantiate()
	mapper = load("res://Mapper/Mapper.tscn").instantiate()
	sfxer = load("res://SFXer/SFXer.tscn").instantiate()
	ES.editor = self
...
func add_window(name, control):
	windows.append(name)
	control.custom_minimum_size = Vector2(320, 240)
	control.name = name
	add_child(control)
