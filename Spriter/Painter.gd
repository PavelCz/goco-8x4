...
func _ready():
	canvas.connect("mouse_position_changed", Callable(self, "_on_mouse_position_changed"))
	canvas.connect("mouse_entered", Callable(status_pixel_pos, "show"))
	canvas.connect("mouse_exited", Callable(status_pixel_pos, "hide"))
